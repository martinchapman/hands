...
public class AdaptiveUtils {
	...
	public static double internalMeasure(ArrayList<Double> roundStrategyPerformance, Metric metric, HiddenObjectGraph<StringVertex, StringEdge> hiddenObjectGraph) {
		...
		if ( metric == Metric.RELATIVE_COST ) {
			...
			Utils.talk("internalMeasure", "Payoff relative: Percentage change " + roundStrategyPerformance.get(0) + " to " + roundStrategyPerformance.get(roundStrategyPerformance.size() - 1));
			...
			Utils.talk("Adaptive Utils", Math.abs(roundStrategyPerformance.get(roundStrategyPerformance.size() - 1)) + " " + Utils.totalEdgeCost(hiddenObjectGraph) + " " + Math.abs(roundStrategyPerformance.get(roundStrategyPerformance.size() - 1)) / Utils.totalEdgeCost(hiddenObjectGraph));
